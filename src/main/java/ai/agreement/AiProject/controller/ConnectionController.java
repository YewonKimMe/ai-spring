...
public class ConnectionController {
    ...
    @Operation(summary = "인증키 발급", description = "최초 접속 시 브라우저용 인증키 발급<br>테스트 모드에서는 API KEY 를 암호화한 인증키 제공<br>테스트 모드가 아닐경우 JWT Token 생성 후 응답 헤더로 제공, 프론트에서 쿠키로 저장 후 POST 요청 시마다 요청 Authorization 헤더에 포함<br>Jwt 유효기간: 3시간<br><br>테스트모드: false")
    @GetMapping
